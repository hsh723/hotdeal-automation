        ...
        for attempt in range(retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    if items:
                        logger.info("첫 번째 상품 HTML 구조 샘플:")
                        logger.info(str(items[0])[:300] + "...")  # 앞부분만 출력
                    else:
                        logger.warning("상품 목록을 찾을 수 없음")
                        # 페이지에 "와우 플러스 특가" 같은 텍스트가 있는지 확인
                        page_text = soup.get_text()
                        if "와우 특가" in page_text or "핫딜" in page_text:
                            logger.info("페이지는 핫딜 페이지로 보이지만 상품 목록 구조를 찾을 수 없음")
                        continue  # 다음 URL 시도
                    ...
                    for item in items:
                        ...
                        try:
                            # 다양한 제목 셀렉터 시도
                            title_selectors = ["div.name", "div.product-name", "a.product-link", "div.title"]
                            title_elem = None
                            ...
                            # 다양한 가격 셀렉터 시도
                            price_selectors = ["strong.price-value", "strong.price", "em.sale", "div.price-area"]
                            price_elem = None
                            ...
                            price_text = ''.join(filter(str.isdigit, price_text))  # 숫자만 추출
                            price = int(price_text) if price_text else 0
                            
                            # 다양한 원래 가격 셀렉터 시도
                            original_price_selectors = ["del.base-price", "del.original-price", "span.original-price"]
                            ...
                            original_price_text = original_price_elem.text.strip() if original_price_elem else ""
                            original_price_text = ''.join(filter(str.isdigit, original_price_text))  # 숫자만 추출
                            original_price = int(original_price_text) if original_price_text else price
                            ...
                            if discount_elem:
                                ...
                                discount_text = ''.join(filter(str.isdigit, discount_text))  # 숫자만 추출
                            ...
                            for selector in link_selectors:
                                ...
                                for elem in link_elems:
                                    if elem.has_attr("href") and "/vp/" in elem["href"]:
                                        link_elem = elem
                                        break
                                if link_elem:
                                    break
                                    
                            link = "https://www.coupang.com" + link_elem["href"] if link_elem and "href" in link_elem.attrs else ""
                            
                            # 이미지 URL
                            img_selectors = ["img.product-image", "img.search-product-wrap-img", "img"]
                            img_elem = None
                            for selector in img_selectors:
                                img_elems = item.select(selector)
                                for elem in img_elems:
                                    if elem.has_attr("src") and ("jpg" in elem["src"] or "jpeg" in elem["src"] or "png" in elem["src"]):
                                        img_elem = elem
                                        break
                                if img_elem:
                                    break
                            ...
                            # 첫 번째 상품 정보 출력 (디버깅)
                            if len(deals) == 1:
                                logger.info(f"첫 번째 상품 정보: {deals[0]}")
                                
                        except Exception as e:
                            logger.error(f"상품 정보 추출 중 오류: {e}", exc_info=True)
                    ...
                    # 응답 내용의 일부를 출력하여 확인
                    logger.info(f"응답 내용(앞부분): {response.text[:200]}")
